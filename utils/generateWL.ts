...
async function generateHTML(props: SocialLinkProps): Promise<string> {
  ...
  const getIconSvg = (icon: IconType): string => {
  ...
  const socialLinks =
    props.social && props.social.length > 0
      ? props.social
        ...
        >
          <span class="icon-container mr-2 text-2xl flex-shrink-0">${getIconSvg(
            link.icon
          )}</span>
...
<head>
    ...
    <link href="https://arweave.net/drGKSWRDuOdmm-aRpySs2gwhzAznN3ewMO99qhvT8Ts" rel="stylesheet">
</head>
<body>
  <div class="mainM">
