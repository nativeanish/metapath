...
export default async function generateWL(
...
<head>
    ...
    <title>${props.name || "Profile"} - Window Light</title>
...
<body>
    <div class="mainM flex items-center justify-center h-screen text-black bg-[#dfe5f2]">
      <div class="outline-border rounded-md grid h-[800px] max-h-[100dvh] w-[1000px] max-w-[1000px] grid-cols-[100px_auto] rounded-base shadow-[10px_10px_0_0_#000] outline outline-4 w600:grid-cols-[70px_auto] w500:grid-cols-1 portrait:h-[100dvh]">
        ...
        <main class="bg-[#e3dff2] relative flex h-[800px] max-h-[100dvh] flex-col rounded-br-base rounded-tr-base font-semibold portrait:h-[100dvh] portrait:max-h-[100dvh] portrait:rounded-none">
          <div class="main h-full max-h-full overflow-y-auto portrait:max-h-full bg-[#e3dff2]">
            <div class="border-b-border bg-[#a388ee] h-[50px] rounded-tr-base border-b-4 border-black text-xl w600:text-lg w400:h-10 w400:text-base portrait:rounded-none">
              <div class="flex h-full items-center justify-center uppercase text-black">
                Profile
              </div>
            </div>
            <div class="flex flex-col md:flex-row items-center md:items-start md:space-x-10 text-black space-y-4 md:space-y-0 p-4 md:p-10 text-lg leading-[1.7] w600:text-lg w400:text-base">
              ${
                props.image
                  ? `
                <img
