...
function _generateCD(props: SocialLinkProps): string {
  const generateClassicButton = (
    ...
    return `
      <a 
        onclick="look('${uuid}')" 
        class="${className} transform hover:scale-105 transition-all duration-300 rounded-lg flex flex-row items-center justify-center space-x-4 p-3 font-medium shadow-sm hover:shadow-md mb-3" 
        target="_blank" 
        rel="noopener noreferrer" 
        href="${href}"
      >
        <div class="h-6 w-6 sm:h-6 sm:w-6 flex items-center justify-center">
          <img src="https://arweave.net/${icon_address?.arweave[0]}" alt="${text}" class="w-full h-full">
        </div>
...
<body class="min-h-screen bg-[#212121] text-white py-12 px-4 sm:px-6 lg:px-8">
  <button
    onclick="handleShare()"
    class="fixed border-2 border-white top-4 right-4 md:top-8 md:right-8 z-10 flex items-center justify-center px-4 py-2 glass-effect text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
  >
  ...
  <div class="max-w-md mx-auto pt-3">
    <div class="text-center space-y-8">
      <div class="w-32 h-32 rounded-2xl mx-auto flex items-center justify-center transform hover:rotate-3 transition-transform duration-300 shadow-lg">
        ${
          props.image
            ? `
          <img
            class="w-full h-full object-cover rounded-2xl bg-white border-white border-2"
            src="https://arweave.net/${props.image}"
      ...
      <div class="space-y-4">
        <h1 class="text-5xl font-bold text-white tracking-tight hover:tracking-wide transition-all duration-300">
          ${props.name || ""}
        </h1>
        <p class="mt-5 text-md text-gray-300"> ${
          props.description && props.description.length ? props.description : ""
        } </p>
      </div>
    </div>
  ...
  <div class="mt-20 max-w-xs mx-auto text-center">
    <div class="glass-effect rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
      <h2 class="text-xl font-bold text-white mb-6">Create Your Own Page</h2>
      <a
...
export default async function generateCD(props: SocialLinkProps) {
  try {
    const htmlOutput = _generateCD(props);
    return htmlOutput;
  } catch (error) {
